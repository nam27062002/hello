...
public class DragonAutoZoom : MonoBehaviour {
	...
	void OnEnable() {
		...
		Messenger.AddListener<DragonPlayer.EState, DragonPlayer.EState>(GameEvents.PLAYER_STATE_CHANGED, OnPlayerStateChanged);
	}
	...
	void OnDisable() {
		// Subscribe to external events
		Messenger.RemoveListener<DragonPlayer.EState, DragonPlayer.EState>(GameEvents.PLAYER_STATE_CHANGED, OnPlayerStateChanged);
	}
