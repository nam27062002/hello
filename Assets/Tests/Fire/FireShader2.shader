Shader "Hungry Dragon/FireShader2"
{
	Properties
	{
...
//				i.uv.y *= 1.0 - i.uv.y;
				i.uv -= half2(0.5, 0.5);
				float st = _ShaderTime - _Time.y;
				float t1 = frac((st * _Speed) + 0.333333);
				float t2 = frac((st * _Speed) + 0.666666);
				float t3 = frac((st * _Speed) + 0.999999);
...
//				intensity += noise(uv3 + _Seed) * (1.0 - i3);
				...
				float intensity;
				intensity  = tex2D(_NoiseTex, (uv1 + _Seed * 1.0)).x * (1.0 - i1);
				intensity += tex2D(_NoiseTex, (uv2 + _Seed * 2.0)).x * (1.0 - i2);
				intensity += tex2D(_NoiseTex, (uv3 + _Seed * 3.0)).x * (1.0 - i3);
