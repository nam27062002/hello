...
[CustomEditor(typeof(BirdBehaviour))]
public class BirdBehaviourEditor : Editor {

	private BirdBehaviour m_target;
	private bool m_isTargetDirty;

	public void Awake() {
		m_target = (BirdBehaviour)target;
	...
	private void DrawHandle() {
		...
		// Rotation
		if (m_target.fleeForce > 0) {
			float alphaFactor = Mathf.Min(1f, m_target.fleeForce);
			float angle = m_target.sensorAngle;

			Vector3 from;
			Vector3 to;
			if (m_target.direction.x < 0) {
				from = Vector3.right.RotateXYDegrees(-m_target.sensorAngle * 0.5f + 180f);
				to = Vector3.right.RotateXYDegrees(m_target.sensorAngle * 0.5f + 180f);
			} else {
