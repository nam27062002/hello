...
/// </summary>
[CustomPropertyDrawer(typeof(Spawner.SpawnCondition))]
public class SpawnerSpawnConditionPropertyEditor : ExtendedPropertyDrawer {
	//------------------------------------------------------------------//
	// MEMBERS															//
	//------------------------------------------------------------------//
	...
	/// Default constructor.
	/// </summary>
	public SpawnerSpawnConditionPropertyEditor() {
		// Nothing to do
	}
	...
	override protected void OnGUIImpl(SerializedProperty _property, GUIContent _label) {
		...
		m_pos.height = Mathf.Max(EditorGUI.GetPropertyHeight(typeProp), EditorGUI.GetPropertyHeight(valueProp));
		...
		EditorGUI.PropertyField(pos, typeProp, new GUIContent(), true);
