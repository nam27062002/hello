...
public class HelicopterProjectile :  MonoBehaviour, IProjectile { 
	...
	public void Shoot(Transform _target, Vector3 _direction, float _damage, Transform _source) {
        m_pMotion.m_moveType = ProjectileMotion.Type.Missile;
	...
	public void ShootTowards(Vector3 _direction, float _speed, float _damage, Transform _source) {
        if (m_pMotion != null) {
            ...
            m_pMotion.m_moveType = ProjectileMotion.Type.Arrow;
            m_pMotion.Shoot( transform.position + _direction );
        }
