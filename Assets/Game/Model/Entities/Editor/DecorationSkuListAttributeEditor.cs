// DecorationSkuListAttributeEditor.cs
// 
...
/// </summary>
[CustomPropertyDrawer(typeof(DecorationSkuListAttribute))]
public class DecorationSkuListAttributeEditor : ExtendedPropertyDrawer {
	//------------------------------------------------------------------//
	// MEMBERS															//
	//------------------------------------------------------------------//
	...
	override protected void OnGUIImpl(SerializedProperty _property, GUIContent _label) {
		...
		for(int i = 0; i < categorySkus.Count; i++) {
			defsByCategory.Add(categorySkus[i], DefinitionsManager.SharedInstance.GetDefinitionsByVariable(DefinitionsCategory.DECORATIONS, "category", categorySkus[i]));
		}
