...
public class DragonSkill : SerializableClass {
	...
	public DragonSkill(DragonData _owner, string _skillSku) {
		...
		m_def = DefinitionsManager.SharedInstance.GetDefinition(DefinitionsCategory.DRAGON_SKILLS, _skillSku);
		Debug.Assert(m_def != null, "Skill " + _skillSku + " not recognized!");

		m_progressionDef = DefinitionsManager.SharedInstance.GetDefinition(DefinitionsCategory.DRAGON_SKILLS, _owner.def.sku);	// Shares sku with the dragons
