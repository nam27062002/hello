...
public class GlobalEventsPanelActive : GlobalEventsPanel {
	...
	private void UpdatePeriodic() {
		...
		double remainingTime = System.Math.Max(0, HDLiveDataManager.instance.m_quest.m_questData.remainingTime.TotalSeconds);
		...
		if(m_updateEventState) {
			if ( remainingTime <= 0 )
			{
				HDLiveDataManager.instance.m_quest.UpdateStateFromTimers();
			}

			if ( !HDLiveDataManager.instance.m_quest.IsRunning() )
			{
