        ...
        if (m_unavailableInfoText != null)
        {
            ...
            if (show)
            {
                ...
                else
                {
                    ...
                    DefinitionNode tierDef = DefinitionsManager.SharedInstance.
                                                                GetDefinition(DefinitionsCategory.DRAGON_TIERS,
                                                                IDragonData.TierToSku(unlockTier));
                    if (unlockTier != DragonTier.COUNT)
                    {
                        // Set text
                        m_unavailableInfoText.Localize(
                            "TID_SELECT_DRAGON_UNLOCK_BY_TIER",    // Obtain %U0 to reveal this dragon's price!
                            UIConstants.GetTierTag(tierDef) // Use tier's icon
                        );
                    }
                }
            }
