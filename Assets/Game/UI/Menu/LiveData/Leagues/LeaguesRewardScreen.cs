...
/// </summary>
public class LeaguesRewardScreen : MonoBehaviour {	
	//------------------------------------------------------------------//
	// CONSTANTS														//
	//------------------------------------------------------------------//
	private enum State {
	...
	public void StartFlow() {
		if(m_state == State.FLOW_NOT_STARTED) {
			...
			if(m_leagueIcon != null) {
				m_leagueIcon.sprite = Resources.Load<Sprite>(UIConstants.LIVE_EVENTS_ICONS_PATH + m_tournamentDef.m_goal.m_icon);
			}
			if(m_goalText != null) {
