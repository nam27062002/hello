...
//----------------------------------------------------------------------//
/// <summary>
/// Custom editor for the SeasonTrigger.SeasonDictionary class.
/// </summary>
//[CustomPropertyDrawer(typeof(SeasonTrigger.SeasonDictionary), true)]
//public class SeasonDictionaryEditor : SerializableDictionaryEditor { }

/// <summary>
/// Custom editor for the SeasonTrigger class.
/// </summary>
[CustomEditor(typeof(SeasonTrigger), true)]	// True to be used by heir classes as well
[CanEditMultipleObjects]
public class SeasonTriggerEditor : Editor {
	//------------------------------------------------------------------------//
	// CONSTANTS															  //
	//------------------------------------------------------------------------//
	...
	SeasonTrigger m_targetSeasonTrigger = null;
	...
	private void OnEnable() {
		// Get target object
		m_targetSeasonTrigger = target as SeasonTrigger;
	}
		...
		if(seasonProp.isExpanded) {
			...
			for(int i = 0; i < targetsProp.arraySize; ++i) {
				...
				EditorGUILayout.BeginHorizontal(); {
					// Object reference
					p = targetProp.FindPropertyRelative("obj");
					p.objectReferenceValue = EditorGUILayout.ObjectField(p.objectReferenceValue, typeof(GameObject), true);

					// Action
					GUILayout.Space(-15f);	// Remove useless space
					p = targetProp.FindPropertyRelative("action");
					EditorGUILayout.PropertyField(p, GUIContent.none, true, GUILayout.Width(85f));
