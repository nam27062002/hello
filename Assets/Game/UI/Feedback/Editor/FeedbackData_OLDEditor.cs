...
namespace LevelEditor {
	...
	/// </summary>
	[CustomEditor(typeof(FeedbackData_OLD))]
	public class FeedbackData_OLDEditor : Editor {
		//------------------------------------------------------------------//
		// CONSTANTS														//
		//------------------------------------------------------------------//
		...
		// PROPERTIES														//
		//------------------------------------------------------------------//
		private FeedbackData_OLD targetData { get { return target as FeedbackData_OLD; }}
		...
		public override void OnInspectorGUI() {
			// Aux vars
			int numTypes = (int)FeedbackData_OLD.Type.COUNT;
			...
			for(int i = 0; i < numTypes; i++) {
				...
				EditorGUILayout.BeginHorizontal(); {
					// Foldable
					probabilityProp.isExpanded = EditorGUILayout.Foldout(probabilityProp.isExpanded, ((FeedbackData_OLD.Type)i).ToString());
				...
				if(probabilityProp.isExpanded) {
					...
					if(actualListProp.arraySize == 0) {
						...
						style.wordWrap = true;
						EditorGUILayout.LabelField("", "Feedbacks list for " + ((FeedbackData_OLD.Type)i).ToString() + " is empty. Add some with the \"+\" button.", style);
					} else {
						// Current entries
