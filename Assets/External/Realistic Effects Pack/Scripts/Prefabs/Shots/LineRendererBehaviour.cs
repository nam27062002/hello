	...
	void InitializeDefault() {
		...
		if (IsVertical) {
			if (Physics.Raycast(tRoot.position, Vector3.down, out hit)) {
				...
				if (GoLight != null)
					GoLight.transform.position = hit.point + new Vector3(0, LightHeightOffset, 0);
				if (Particles != null)
					Particles.transform.position = hit.point + new Vector3(0, ParticlesHeightOffset, 0);
				if (Explosion != null)
					Explosion.transform.position = hit.point + new Vector3(0, ParticlesHeightOffset, 0);
			}
		} else {
			...
			line.SetPosition(1, hitPoint - effectSettings.ColliderRadius * direction);
			var particlesOffsetPos = hitPoint - direction * ParticlesHeightOffset;
			if (StartGlow != null)
				StartGlow.transform.position = tRoot.position;
			if (HitGlow != null)
				HitGlow.transform.position = hitPoint;//particlesOffsetPos;
			if (GoLight != null)
				GoLight.transform.position = hitPoint - direction * LightHeightOffset;
			if (Particles != null)
				Particles.transform.position = hitPoint;//particlesOffsetPos;
			if (Explosion != null) {
				Explosion.transform.position = hitPoint;//particlesOffsetPos;
				Explosion.transform.LookAt(hitPoint/*particlesOffsetPos*/ + hit.normal);
			}
		}
