...
namespace TMPro
{
	...
	public enum TextureMappingOptions { Character = 0, Line = 1, Paragraph = 2, MatchAspect = 3, CharacterRandom = 4 };	// [AOC] ADDITION
    ...
    public class TMP_Text : MaskableGraphic
    {
		...
		// [AOC] ADDITIONS
		// For random UV mapping, store and serialize the random seed so that the mapping looks always the same within the same run.
		protected UnityEngine.Random.State? m_mappingRandomState = null;	// [AOC] Use ? to make the struct nullable (http://stackoverflow.com/questions/6564682/how-to-create-structure-with-null-value-support)
        ...
        protected virtual Vector2 CalculatePreferredValues(float defaultFontSize, Vector2 marginSize, bool ignoreTextAutoSizing)
        {
            ...
            for (int i = 0; m_char_buffer[i] != 0; i++)
            {
                ...
                float elementAscender = m_currentFontAsset.fontInfo.Ascender * (m_textElementType == TMP_TextElementType.Character ? currentElementScale : m_internalCharacterInfo[m_characterCount].scale) + m_baselineOffset;
                ...
                float elementDescender = m_currentFontAsset.fontInfo.Descender * (m_textElementType == TMP_TextElementType.Character ? currentElementScale : m_internalCharacterInfo[m_characterCount].scale) + m_baselineOffset;
        ...
        protected float ConvertToFloat(char[] chars, int startIndex, int length, out int lastIndex)
        {
            ...
            for (int i = startIndex; i < endIndex; i++)
            {
                ...
                if (c == 32) continue;
                if (c == 46) { decimalplace = i; digit = -1; continue; }
                if (c == 45) { sign = -1; continue; }
                if (c == 43) { sign = 1; continue; }
                if (c == 44) { lastIndex = i; return value * sign; }

                // Return if char is not a digit.
                if (!char.IsDigit(c)) { lastIndex = i; return -9999; }
                ...
                switch (digit)
                {
                    ...
                    case 6:
                        ...
                        break;
                    case -1:
                        value = value + (chars[i] - 48) * 0.1f;
                        break;
                    case -2:
                        value = value + (chars[i] - 48) * 0.01f;
                        break;
                    case -3:
                        value = value + (chars[i] - 48) * 0.001f;
                        break;
                    case -4:
                        value = value + (chars[i] - 48) * 0.0001f;
                        break;
                    case -5:
                        value = value + (chars[i] - 48) * 0.00001f;
                        break;
                }
