...
namespace TMPro
{
	...
	public partial class TextMeshPro
	{
		...
		// Used to offset UV on Texturing

		[SerializeField]
		private float m_uvLineOffset = 0.0f;
		// Used for UV line offset per line

		[SerializeField]
		private bool m_hasFontAssetChanged = false;
		// Used to track when font properties have changed.

		float m_previousLossyScaleY = -1;
		// Used for Tracking lossy scale changes in the transform;
		...
		// Flag to determine if this is the first allocation of the buffers.
		private int m_max_characters = 8;
		// Determines the initial allocation and size of the character array / buffer.
		private int m_max_numberOfLines = 4;
		// Determines the initial allocation and maximum number of lines of text.
		...
		// Struct which holds various states / variables used in conjunction with word wrapping.
        ...
        */
		...
		protected override void Awake ()
		{
			...
			if (m_fontSizeMin == 0)
				m_fontSizeMin = m_fontSize / 2;
			if (m_fontSizeMax == 0)
				m_fontSizeMax = m_fontSize * 2;
					...
					if (glyph == null) {
						...
						if (m_currentFontAsset.characterDictionary.TryGetValue (altGlyphCode, out glyph)) {
							if (!TMP_Settings.warningsDisabled)
								Debug.LogWarning ("Character with ASCII value of " + c + " was not found in the Font Asset Glyph Table.", this);
								...
								if (tempFontAsset != null && tempFontAsset.characterDictionary.TryGetValue (altGlyphCode, out glyph)) {
									...
									if (tempFontAsset != null && tempFontAsset.characterDictionary.TryGetValue (altGlyphCode, out glyph)) {
										...
										if (m_currentFontAsset.characterDictionary.TryGetValue (32, out glyph)) {
											// Use Space (32) Glyph
											if (!TMP_Settings.warningsDisabled)
												Debug.LogWarning ("Character with ASCII value of " + c + " was not found in the Font Asset Glyph Table. It was replaced by a space.", this);
											c = chars [i] = 32;
										}
									}
								}
		...
		void OnPreRenderObject ()
		{
			...
			if (!m_isAwake || !m_ignoreActiveState && !this.IsActive ())
				return;
		...
		protected override void GenerateTextMesh ()
		{
			...
			for (int i = 0; m_char_buffer [i] != 0; i++) {
				...
				} else {   // This is a Space, Tab, LineFeed or Carriage Return
			...
			for (int i = 0; i < m_characterCount; i++) {
				...
				case TextAlignmentOptions.CaplineJustified:
					...
					if (currentCharacter == 0xAD || currentCharacter == 0x200B || currentCharacter == 0x2060)
						break;
				...
				if (isCharacterVisible) {
					...
					case TMP_TextElementType.Character:
						...
						case TextureMappingOptions.MatchAspect:
							...
							case TextureMappingOptions.CharacterRandom:
								{
									float offsetY = UnityEngine.Random.Range (0f, 1f);
									characterInfos [i].vertex_BL.uv2.y = 0 + offsetY;
									characterInfos [i].vertex_TL.uv2.y = 0 + offsetY;
									characterInfos [i].vertex_TR.uv2.y = 1 + offsetY;
									characterInfos [i].vertex_BR.uv2.y = 1 + offsetY;
								}
								break;
							}
						...
						case TextureMappingOptions.CharacterRandom:
							{
								float offsetX = UnityEngine.Random.Range (0f, 1f);
								characterInfos [i].vertex_BL.uv2.x = 0 + offsetX;
								characterInfos [i].vertex_TL.uv2.x = 0 + offsetX;
								characterInfos [i].vertex_TR.uv2.x = 1 + offsetX;
								characterInfos [i].vertex_BR.uv2.x = 1 + offsetX;
							} break;
						}
						...
						characterInfos [i].vertex_BL.uv2.x = PackUV (x0, y0);
						characterInfos [i].vertex_BL.uv2.y = xScale;
						characterInfos [i].vertex_TL.uv2.x = PackUV (x0, y1);
						characterInfos [i].vertex_TL.uv2.y = xScale;
						characterInfos [i].vertex_TR.uv2.x = PackUV (x1, y1);
						characterInfos [i].vertex_TR.uv2.y = xScale;
						characterInfos [i].vertex_BR.uv2.x = PackUV (x1, y0);
						characterInfos [i].vertex_BR.uv2.y = xScale;
                            #endregion
						break;
				...
				if (isUnderline) {
					...
					if (beginUnderline && m_characterCount == 1) {
						...
						} else {   // End underline if last character of the line.
							underline_end = new Vector3 (m_textInfo.characterInfo [i].topRight.x, underlineBaseLine, 0);
							underlineEndScale = m_textInfo.characterInfo [i].scale;
						}
