...
public class SoldierBehaviourOLD : MonoBehaviour {
	...
	void Initialize() {
		...
		gun = GetComponent<SoldierGunBehaviour>();
	...
	void Update () {
		...
		if (state == State.IDLE){
			...
			UpdateDir ();
			pos += dir*Time.deltaTime*runSpeed*0.5f;
			...
			anim.SetLayerWeight( 1, Mathf.Clamp(d.y,0f,1f)*0.8f);
		...
		anim.SetBool ("moving", state == State.WALK || state == State.BURN || state == State.REPOSITION);
		anim.SetBool ("running", state == State.REPOSITION);
		anim.SetBool ("shooting", state == State.ATTACK);
