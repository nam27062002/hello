...
public class SaveFacade : AutoGeneratedSingleton<SaveFacade>
{
    ...
    public const string  UploadTimePref = "CloudSave.UploadTime";
    ...
    public void Enable(User.LoginType loginType, Action onSuccessEnable, Action onFailedEnable)
    {
        ...
        Authenticator.Instance.CheckConnection(delegate(Error connectionError)
        {
            ...
            {
                PersistenceManager.Popups_OpenEnableCloudFailed((int)connectionError.code,
                    delegate ()
                    {
                        PersistenceManager.Popups_CloseLoadingPopup();
                        m_enablingCloudSave = false;
    ...
    private void Popups_OpenEnableCloud(bool enable, Action onConfirm, Action onCancel)
    {
        if (enable)
        {
            PersistenceManager.Popups_OpenEnableCloudSavePopup(onConfirm, onCancel);
        }
        else
        {
            PersistenceManager.Popups_OpenDisableCloudSavePopup(onConfirm);
        }
    }
    #endregion
    ...
    private void ContinueLocally(bool cloudSaveAvailable)
    {
        ...
        switch (result)
        {
            ...
            case LoadState.PermissionError:
                PersistenceManager.Popups_OpenErrorLoadFailed(
                    delegate ()
                    {
                        ContinueLocally(cloudSaveAvailable);
                    }
            ...
            default:
                ...
                Debug.Log("SaveFacade :: Default result: TODO: Show Popup");
                break;
        }
    }
    ...
    private void Login(User.LoginType loginType, bool enableFlow, bool continueLocally = true, Action onSuccess = null, Action onFailed = null)
    {
        ...
        AuthManager.Instance.Login(loginType, new PermissionType[] { PermissionType.Basic }, delegate(Error error, PermissionType[] grantedPermissions, bool cloudSaveAvailable)
        {
            ...
            else
            {
                ...
                {
                    PersistenceManager.Popups_OpenLoginToCloudFailed((int)error.code, SocialManager.GetSelectedSocialNetwork(),
                        delegate ()
                        {
    ...
    private void UpdateNeeded(bool upgradeAvailable, bool localCorrupt = false, Action onFailed = null)
    {
        ...
        /*if (localCorrupt)
        {
            string title = "STRING_SAVE_POPUP_ERROR_LOCAL_SAVE_CORRUPTED_TITLE";

            if(upgradeAvailable)
            {                                
                MessageBoxPopup.MessageBoxConfig config = new MessageBoxPopup.MessageBoxConfig();
                config.titleText = title;
                config.messageText = "STRING_SAVE_POPUP_ERROR_LOCAL_SAVE_CORRUPTED_TEXT_UPDATE1";
            ...
            {                             
                MessageBoxPopup.MessageBoxConfig config = new MessageBoxPopup.MessageBoxConfig();
                config.titleText = title;
                config.messageText = "STRING_SAVE_POPUP_ERROR_LOCAL_SAVE_CORRUPTED_TEXT_UPDATE2";
                config.onConfirm = delegate()
                {
                    SaveGameManager.Instance.CreateNewSave(Authenticator.Instance.User);
        ...
        {
            string title = "STRING_SAVE_POPUP_ERROR_UPDATE_TITLE";

            if(upgradeAvailable)
            {                                
                MessageBoxPopup.MessageBoxConfig config = new MessageBoxPopup.MessageBoxConfig();
                config.titleText = title;
                config.messageText = "STRING_SAVE_POPUP_ERROR_UPDATE_TEXT1";
            ...
            {                
                MessageBoxPopup.MessageBoxConfig config = new MessageBoxPopup.MessageBoxConfig();
                config.titleText = title;
                config.messageText = "STRING_SAVE_POPUP_ERROR_UPDATE_TEXT2";
                config.onConfirm = delegate()
                {
                    if (onFailed != null)
                    {
    ...
    private void SyncCloudSave()
    {
                ...
                if(error == null)
                {
					...
					switch(resultState)
                    {
                        ...
                        case SyncState.PermissionError:
                            ...
                            PersistenceManager.Popups_OpenLoadSavePermissionError(
                                delegate ()
                                {
                                    ContinueLocally();
                                }
        ...
        if(AuthManager.Instance.upgradeAvailable)
        {
            ...
            //[DGR] UPDATE No support added yet
            MessageBoxPopup.MessageBoxConfig config = new MessageBoxPopup.MessageBoxConfig();
            config.titleText = "STRING_SAVE_POPUP_PROMPT_UPDATE_TITLE";
            ...
            config.messageText = platformUpdateMessage;
            config.confirmText = "STRING_BUTTON_UPDATE";
            config.onConfirm = delegate()
            {
                GoToAppStore();
            ...
            */
            ...
            Debug.Log("SaveFacade :: Upgrade available: TODO: Show Popup");
        }
        else
        {
            sync();
